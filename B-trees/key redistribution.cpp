...
bool distribution(BLPTR p)
{
	...
	i++;
	if(i-1>=0 && i+1!=3)
	return emptyslot(pp->ptr[i-1])||emptyslot(pp->ptr[i+1]);
	else if(i-1>=0)
	return emptyslot(pp->ptr[i-1]);
	else
	return emptyslot(pp->ptr[i+1]);
}
void print(BLPTR T)
...
void keyredistribution(BLPTR p,int n)
{
